...
function openHudMenu() {
    Inventory.Close();
    $.post('https://qbr-inventory/OpenHudMenu');
}
...
//                         //     Inventory.Close();
//                         // }
//                         $.post("https://qbr-inventory/UseItem", JSON.stringify({
//                             inventory: "player",
//                             item: item,
//                         }));
//                     }
...
$(document).on("dblclick", ".item-slot", function (e) {
    ...
    if (ItemData) {
        Inventory.Close();
        $.post("http://qbr-inventory/UseItem", JSON.stringify({
...
function FormatAttachmentInfo(data) {
    $.post("http://qbr-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function (event, ui) {
            $.post('http://qbr-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function (e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post('http://qbr-inventory/Notify', JSON.stringify({
...
function handleDragDrop() {
    ...
    $("#item-drop").droppable({
        ...
        drop: function (event, ui) {
            ...
            $.post("http://qbr-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
                amount: parseInt(amount),
            }));
...
$(document).on('click', '.CombineItem', function (e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('http://qbr-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('http://qbr-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("http://qbr-inventory/SetInventoryData", JSON.stringify({
...
(() => {
    ...
    Inventory.Error = function () {
        $.post("http://qbr-inventory/PlayDropFail", JSON.stringify({}));
    }
    ...
    Inventory.Close = function () {
        ...
        $.post("http://qbr-inventory/CloseInventory", JSON.stringify({}));
    ...
    Inventory.Clickables = function () {
        ...
        $('.cloth').find('img').click(function () {
            $.post("http://qbr-inventory/ChangeComponent", JSON.stringify({
    ...
    Inventory.Necessary = function () {
        ...
        $('#clothing-menu').click(function () {
            if (isClothOpen == false) {
                isClothOpen = true
                $.post("http://qbr-inventory/OpenClothMenu", JSON.stringify({ delete: isClothOpen }));
...
$(document).on('click', '#rob-money', function (e) {
    ...
    $.post('http://qbr-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});
